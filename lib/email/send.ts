...
const FROM = process.env.RESEND_FROM || "tracable <onboarding@resend.dev>";
...
export async function sendInviteEmail(args: {
  ...
  const html = `
    <div style="font-family: -apple-system, system-ui, sans-serif; max-width: 520px; margin: 0 auto; color: #1c1917">
      <h1 style="font-family: Georgia, serif; font-size: 28px; margin: 0 0 16px">You're invited to ${escape(args.orgName)} on tracable</h1>
  ...
  const res = await c.emails.send({
    from: FROM,
    to: args.to,
    subject: `Join ${args.orgName} on tracable`,
    html,
  });
